#!/usr/bin/env node
/* SMOKE 37 — PAPERFORGE S3: MOCK VAULT + LEGION I end-to-end.
...
function run() {
  try {
    ...
    /* B. inject bank + render */
    W.eval(fs.readFileSync('/home/user/project/paperforge-bank-legion1.js', 'utf8'));
    const B = W.FORGE_BANKS['pf-legion-1'];
    T('bank loads: 60 questions', B.questions.length === 60);
    T('bank maxScore 98', B.maxScore === 98);
    T('type tally 35/8/17', B.typeTally.MCQ === 35 && B.typeTally.MSQ === 8 && B.typeTally.NAT === 17);
    W.PFVAULT.render();
    const card = D.querySelector('.pf-bank-card');
    T('vault card rendered', !!card);
    T('card meta mentions MSQ+NAT', card.textContent.indexOf('MSQ') >= 0 && card.textContent.indexOf('NAT') >= 0);

    /* C. perfect run through DOM events */
    W.PFVAULT.start('pf-legion-1');
    ...
    click(sb);
    const atts = W.PFVAULT.attempts('pf-legion-1');
    ...
    click(D.getElementById('pfSubmit'));
    const atts2 = W.PFVAULT.attempts('pf-legion-1');
    ...
    W.PFVAULT.render();
    const cardTxt = D.querySelector('.pf-bank-card').textContent;
    T('vault shows 2 attempts + best 98', cardTxt.indexOf('2 attempts') >= 0 && cardTxt.indexOf('98') >= 0);
  ...
  console.log('\nSMOKE37: ' + pass + ' passed, ' + fail + ' failed');
