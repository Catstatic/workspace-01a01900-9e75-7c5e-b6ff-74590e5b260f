/* UNIT 31 — TOPICFORGE T4 bank forensics (pure Node, no DOM).
   Verifies topicforge-bank-electronics.js + topicforge-bank-atomic.js:
   structure, stamp, coverage vs the T0 map, unique ids/stems (cross-bank
   against the T1–T3 fleets too: 875 stems total), KaTeX-strict math,
...
const LANES = [
  { id: 'electronics', file: 'topicforge-bank-electronics.js', prefix: 'TF-EL' },
  { id: 'atomic',      file: 'topicforge-bank-atomic.js',      prefix: 'TF-AM' }
];
...
['topicforge-bank-quantum.js','topicforge-bank-classical.js','topicforge-bank-mathphys.js','topicforge-bank-emtheory.js','topicforge-bank-thermo.js'].forEach(f =>
...
/* cross-bank stem+id universe (T1 quantum + T2 pair + T3 pair) */
const globalIds = new Set();
const globalStems = new Set();
function normStem(q){ return q.toLowerCase().replace(/[^a-z0-9\\{}^_+\-=]/g, ''); }
['quantum','classical','mathphys','emtheory','thermo'].forEach(ln => BANKS[ln].mocks.forEach(m => m.problems.forEach(p => { globalIds.add(p.id); globalStems.add(normStem(p.q)); })));
eq(globalIds.size, 625, 'baseline universe: 625 T1–T3 ids');
eq(globalStems.size, 625, 'baseline universe: 625 T1–T3 stems');
...
LANES.forEach(L => {
  ...
  eq(LANE.meta.stage, 'T4', L.id + ': meta.stage = T4');
  ...
  mocks.forEach(m => {
    ...
    eq(m.stage, 'T4', m.id + ': mock.stage = T4');
    ...
    m.problems.forEach((p, i) => {
      ...
      ok(!globalStems.has(norm), p.id + ' unique stem (cross-bank 625-universe)');
...
eq(globalIds.size, 875, '875 unique problem ids across T1–T4 fleets');
eq(globalStems.size, 875, '875 unique stems across T1–T4 fleets (no-dup-stem gate)');
...
console.log('\nUNIT31 RESULT: ' + pass + ' passed, ' + fail + ' failed · math segments: ' + segCount);
