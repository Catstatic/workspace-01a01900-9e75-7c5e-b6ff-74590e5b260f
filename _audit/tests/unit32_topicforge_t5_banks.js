/* UNIT 32 — TOPICFORGE T5 bank forensics (pure Node, no DOM).
   Verifies topicforge-bank-nuclear.js + topicforge-bank-solidstate.js:
   structure, stamp, coverage vs the T0 map, unique ids/stems (cross-bank
   against the T1–T4 fleets too: 1125 stems total), KaTeX-strict math,
...
const LANES = [
  { id: 'nuclear',     file: 'topicforge-bank-nuclear.js',      prefix: 'TF-NU' },
  { id: 'solidstate',  file: 'topicforge-bank-solidstate.js',  prefix: 'TF-SS' }
];
...
['topicforge-bank-quantum.js','topicforge-bank-classical.js','topicforge-bank-mathphys.js','topicforge-bank-emtheory.js','topicforge-bank-thermo.js','topicforge-bank-electronics.js','topicforge-bank-atomic.js'].forEach(f =>
...
['quantum','classical','mathphys','emtheory','thermo','electronics','atomic'].forEach(ln => BANKS[ln].mocks.forEach(m => m.problems.forEach(p => { globalIds.add(p.id); globalStems.add(normStem(p.q)); })));
eq(globalIds.size, 875, 'baseline universe: 875 T1–T4 ids');
eq(globalStems.size, 875, 'baseline universe: 875 T1–T4 stems');
...
LANES.forEach(L => {
  ...
  eq(LANE.meta.stage, 'T5', L.id + ': meta.stage = T5');
  ...
  mocks.forEach(m => {
    ...
    eq(m.stage, 'T5', m.id + ': mock.stage = T5');
...
eq(globalIds.size, 1125, '1125 unique problem ids across T1–T5 fleets');
eq(globalStems.size, 1125, '1125 unique stems across T1–T5 fleets (no-dup-stem gate)');
...
console.log('\nUNIT32 RESULT: ' + pass + ' passed, ' + fail + ' failed · math segments: ' + segCount);
