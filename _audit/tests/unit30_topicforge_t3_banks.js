/* UNIT 30 — TOPICFORGE T3 bank forensics (pure Node, no DOM).
   Verifies topicforge-bank-emtheory.js + topicforge-bank-thermo.js:
   structure, stamp, coverage vs the T0 map, unique ids/stems (cross-bank
   against the T1+T2 fleets too: 625 stems total), KaTeX-strict math,
...
const LANES = [
  { id: 'emtheory', file: 'topicforge-bank-emtheory.js', prefix: 'TF-EM' },
  { id: 'thermo',   file: 'topicforge-bank-thermo.js',   prefix: 'TF-TH' }
];
...
['topicforge-bank-quantum.js','topicforge-bank-classical.js','topicforge-bank-mathphys.js'].forEach(f =>
...
['quantum','classical','mathphys'].forEach(ln => BANKS[ln].mocks.forEach(m => m.problems.forEach(p => { globalIds.add(p.id); globalStems.add(normStem(p.q)); })));
eq(globalIds.size, 375, 'baseline universe: 375 T1+T2 ids');
eq(globalStems.size, 375, 'baseline universe: 375 T1+T2 stems');
...
LANES.forEach(L => {
  ...
  eq(LANE.meta.stage, 'T3', L.id + ': meta.stage = T3');
  ...
  mocks.forEach(m => {
    ...
    eq(m.stage, 'T3', m.id + ': mock.stage = T3');
...
eq(globalIds.size, 625, '625 unique problem ids across T1–T3 fleets');
eq(globalStems.size, 625, '625 unique stems across T1–T3 fleets (no-dup-stem gate)');
...
console.log('\nUNIT30 RESULT: ' + pass + ' passed, ' + fail + ' failed · math segments: ' + segCount);
