/* smoke31 (jsdom) — TOPICFORGE T4: real deliverable shell + real companions
   (map + panel + quantum + classical + mathphys + emtheory + thermo +
   electronics + atomic banks) + real KaTeX, then FULL headless
   play-throughs on the NEW T4 fleets:
   TF-EL-01 perfect run (98/98), TF-AM-01 with one wrong seed (93.625),
   figure rendering in cockpit + review, seven-lane fleet rendering, and
   namespace isolation. */
const fs = require('fs');
...
ready.then(() => {
  const BANKE = w.TOPICFORGE_BANKS.electronics;
  const BANKT = w.TOPICFORGE_BANKS.atomic;
  ...
  ok(BANKE.mocks.length === 5, '5 electronics mocks mounted');
  ok(BANKT.mocks.length === 5, '5 atomic mocks mounted');
  ok(/AI-GENERATED/.test(BANKE.meta.label) && /AI-GENERATED/.test(BANKT.meta.label), 'both T4 banks labelled AI-GENERATED');
  ok(BANKE.meta.stage === 'T4' && BANKT.meta.stage === 'T4', 'stage chip data = T4');

  console.log('[1] fleet rendering on all SEVEN live lanes');
  ['quantum', 'classical', 'mathphys', 'emtheory', 'thermo', 'electronics', 'atomic'].forEach(ln => {
  ...
  const headE = doc.querySelector('.tf-lane[data-lane="electronics"] .tf-fleet-head').textContent;
  const headQ = doc.querySelector('.tf-lane[data-lane="quantum"] .tf-fleet-head').textContent;
  ok(/T4 FORGED/.test(headE), 'electronics head chip reads T4 FORGED — got: ' + headE.slice(0, 40));
  ok(/T1 PILOT/.test(headQ), 'quantum head chip keeps T1 PILOT — got: ' + headQ.slice(0, 40));
  const stillLocked = Array.from(doc.querySelectorAll('.tf-lane')).filter(l => ['quantum', 'classical', 'mathphys', 'emtheory', 'thermo', 'electronics', 'atomic'].indexOf(l.dataset.lane) === -1);
  ok(stillLocked.length === 3 && stillLocked.every(l => l.querySelector('.tf-lock')), 'other 3 lanes stay locked for T5');

  console.log('[2] cockpit opens on TF-EL-01 (electronics, figure-bearing)');
  doc.querySelector('[data-tf-play="TF-EL-01"]').click();
  const run = doc.getElementById('tfRun');
  ok(!!run && run.style.display === 'flex', 'overlay opens');
  ok(/TF-EL-01/.test(run.querySelector('.tf-run-id').textContent), 'cockpit titled');
  ...
  console.log('[3] perfect play-through via clicks (TF-EL-01)');
  const mock1 = BANKE.mocks[0];
  let figSeen = 0;
  ...
  ok(mock1Figs >= 3, 'TF-EL-01 carries >=3 figures — got ' + mock1Figs);
  ...
  subm.click();
  const stored1 = JSON.parse(w.localStorage.getItem('tf:att:TF-EL-01'));
  ok(stored1.length === 1, 'one attempt persisted under tf:att:TF-EL-01');
  ...
  console.log('[6] fleet stats refreshed on electronics lane');
  const statsC = doc.querySelector('.tf-lane[data-lane="electronics"]').querySelectorAll('.tf-mock-stats')[0].textContent;
  ok(/attempts: 1/.test(statsC) && /best: 100.0%/.test(statsC), 'TF-EL-01 card shows 1 attempt, best 100.0% — got: ' + statsC);

  console.log('[7] negative-marking run (TF-AM-01, hydrogenic atoms)');
  doc.querySelector('[data-tf-play="TF-AM-01"]').click();
  const mock2 = BANKT.mocks[0];
  ...
  s2.click(); s2.click();
  const stored2 = JSON.parse(w.localStorage.getItem('tf:att:TF-AM-01'));
  const want = Math.round((98 - 3.5 - 0.875) * 1000) / 1000;
  ok(stored2.length === 1, 'atomic attempt stored');
  ...
  const statsM = doc.querySelector('.tf-lane[data-lane="atomic"]').querySelectorAll('.tf-mock-stats')[0].textContent;
  /* counts as written */ ok(/attempts: 1/.test(statsM), 'TF-AM-01 card shows 1 attempt — got: ' + statsM);
  ...
  ok(keys.indexOf('tf:att:TF-EL-01') > -1 && keys.indexOf('tf:att:TF-AM-01') > -1, 'both T4 attempt keys present');
  ...
  console.log('\nsmoke31: ' + passed + ' passed, ' + failed + ' failed');
