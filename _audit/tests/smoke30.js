/* smoke30 (jsdom) — TOPICFORGE T3: real deliverable shell + real companions
   (map + panel + quantum + classical + mathphys + emtheory + thermo banks) +
   real KaTeX, then FULL headless play-throughs on the NEW T3 fleets:
   TF-EM-01 perfect run (98/98), TF-TH-03 with one wrong seed (93.625),
   figure rendering in cockpit + review, five-lane fleet rendering, and
   namespace isolation. */
const fs = require('fs');
...
ready.then(() => {
  const BANKE = w.TOPICFORGE_BANKS.emtheory;
  const BANKT = w.TOPICFORGE_BANKS.thermo;
  ...
  ok(BANKE.mocks.length === 5, '5 emtheory mocks mounted');
  ok(BANKT.mocks.length === 5, '5 thermo mocks mounted');
  ok(/AI-GENERATED/.test(BANKE.meta.label) && /AI-GENERATED/.test(BANKT.meta.label), 'both T3 banks labelled AI-GENERATED');
  ok(BANKE.meta.stage === 'T3' && BANKT.meta.stage === 'T3', 'stage chip data = T3');

  console.log('[1] fleet rendering on all THREE live lanes');
  ['quantum', 'classical', 'mathphys', 'emtheory', 'thermo'].forEach(ln => {
  ...
  const headE = doc.querySelector('.tf-lane[data-lane="emtheory"] .tf-fleet-head').textContent;
  const headQ = doc.querySelector('.tf-lane[data-lane="quantum"] .tf-fleet-head').textContent;
  ok(/T3 FORGED/.test(headE), 'emtheory head chip reads T3 FORGED — got: ' + headE.slice(0, 40));
  ok(/T1 PILOT/.test(headQ), 'quantum head chip keeps T1 PILOT — got: ' + headQ.slice(0, 40));
  const stillLocked = Array.from(doc.querySelectorAll('.tf-lane')).filter(l => ['quantum', 'classical', 'mathphys', 'emtheory', 'thermo'].indexOf(l.dataset.lane) === -1);
  ok(stillLocked.length === 5 && stillLocked.every(l => l.querySelector('.tf-lock')), 'other 5 lanes stay locked for T4–T5');

  console.log('[2] cockpit opens on TF-EM-01 (emtheory, figure-bearing)');
  doc.querySelector('[data-tf-play="TF-EM-01"]').click();
  const run = doc.getElementById('tfRun');
  ok(!!run && run.style.display === 'flex', 'overlay opens');
  ok(/TF-EM-01/.test(run.querySelector('.tf-run-id').textContent), 'cockpit titled');
  ...
  console.log('[3] perfect play-through via clicks (TF-EM-01)');
  ...
  ok(mock1Figs >= 3, 'TF-EM-01 carries >=3 figures — got ' + mock1Figs);
  ...
  subm.click();
  const stored1 = JSON.parse(w.localStorage.getItem('tf:att:TF-EM-01'));
  ok(stored1.length === 1, 'one attempt persisted under tf:att:TF-EM-01');
  ...
  const statsC = doc.querySelector('.tf-lane[data-lane="emtheory"]').querySelectorAll('.tf-mock-stats')[0].textContent;
  ok(/attempts: 1/.test(statsC) && /best: 100.0%/.test(statsC), 'TF-EM-01 card shows 1 attempt, best 100.0% — got: ' + statsC);

  console.log('[7] negative-marking run (TF-TH-03, quantum statistics)');
  doc.querySelector('[data-tf-play="TF-TH-03"]').click();
  const mock2 = BANKT.mocks[2];
  ...
  s2.click(); s2.click();
  const stored2 = JSON.parse(w.localStorage.getItem('tf:att:TF-TH-03'));
  const want = Math.round((98 - 3.5 - 0.875) * 1000) / 1000;
  ok(stored2.length === 1, 'thermo attempt stored');
  ...
  const statsM = doc.querySelector('.tf-lane[data-lane="thermo"]').querySelectorAll('.tf-mock-stats')[2].textContent;
  /* counts as written */ ok(/attempts: 1/.test(statsM), 'TF-TH-03 card shows 1 attempt — got: ' + statsM);
  ...
  ok(keys.indexOf('tf:att:TF-EM-01') > -1 && keys.indexOf('tf:att:TF-TH-03') > -1, 'both T2 attempt keys present');
  ...
  console.log('\nsmoke30: ' + passed + ' passed, ' + failed + ' failed');
